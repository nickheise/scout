...
## Dismissals and the nudge

At a **wave-off** or **wrap/milestone** moment, read `dismissals.md` —
the file next to this one — and follow it exactly. It defines explicit
and inferred dismissal capture (D-008) and the one-time archive nudge;
Gate question (c) consumes the counts and ledger lines it produces.
Never handle a dismissal from memory of this paragraph — the procedure
lives in that file.

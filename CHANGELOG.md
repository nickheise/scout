...
### Changed
- 2026-07-28 — Marketplace renamed `scout` → `nickheise`; MCP server renamed `scout` → `store` (D-024). Fixes the `scout@scout` / `scout:scout` stutter against the ecosystem's own marketplace-named-for-owner convention (`mattpocock/skills` → marketplace `mattpocock`). Install is now `/plugin install scout@nickheise`; `/scout:*` commands are unaffected (governed by D-016's plugin-entry-name lock, not the marketplace name). Updated `.claude-plugin/marketplace.json`, `.mcp.json`, `bin/scout-mcp.mjs`, `test/mcp.test.mjs`, README, `docs/router.md`, `templates/scout-router/SKILL.md`. 241/241 tests green.

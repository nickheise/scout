...
### Fixed
- 2026-07-26 — **Install was completely broken (plugin v0.4.1).** `.claude-plugin/plugin.json` declared `"hooks": "./hooks/hooks.json"`, but Claude Code auto-loads the standard `hooks/hooks.json` by convention — so the manifest reference registered it a second time and the CLI rejected the whole plugin with `Duplicate hooks file detected`. The failure mode was total, not partial: `claude plugin list` reported `Status: × failed to load`, meaning no skills, no hooks, no MCP server — every `/scout:*` verb was unavailable on a fresh install. Fixed by dropping the redundant manifest key (per the plugins reference, `manifest.hooks` is only for *additional* hook files). Reproduced and verified fixed by installing from a local marketplace into an isolated `CLAUDE_CONFIG_DIR`: now `Status: √ enabled`, with all 8 skills and both hooks registered. Version bumped so installed users actually receive the fix — updates only ship when `version` changes.

### Added
- 2026-07-26 — Packaging regression guards (D-023), so the install outage above cannot recur silently: a static assertion that `plugin.json` never re-declares the auto-loaded `hooks/hooks.json` (runs on every CI run, no CLI required); an existence + shape check on `hooks/hooks.json` itself, which nothing guarded once the manifest key was dropped; and a real marketplace install into a throwaway `CLAUDE_CONFIG_DIR` asserting the CLI reports the plugin enabled with all 8 skills and both hooks (self-skips where the `claude` binary is absent). All three verified by reintroducing the bug and confirming they fail.

### Changed
- 2026-07-26 — README rewritten (16.4K → 6.9K chars, −58%). Same substance, plainer language: install moved to the top, the obsolete D-013 "delivery honesty" caveat removed now that Q2 is closed, the tier/update-model/coexistence sections folded into the places they're actually relevant, and a "Where it works" table added covering terminal, desktop **Code** tab, and other agents. Documents desktop support explicitly — plugins and hooks work in the desktop app's Code tab (shared config with the CLI); the Chat tab runs plugin skills but not hooks.
- 2026-07-24 — Surfacing restructured (R1, D-020): dismissal capture and the archive nudge moved to `skills/surfacing/dismissals.md`, loaded only at wave-off/wrap moments — every planning moment now carries ~420 fewer tokens. Verified before landing by five isolated scenario runs (planning gate regression, wave-off, nudge threshold, nudge suppression, wrap-inferred) with 12/12 independent on-disk checks; every dismissal scenario correctly followed the new pointer.
